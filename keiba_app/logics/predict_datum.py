...
class PredictDatum:
  ...
  def predict(race_id: str) -> dict:
    ...
    horse_numbers = race_df['horse_number']
    ...
    if len(horse_df) == len(jockey_df):
      df = pd.concat([jockey_df, horse_df, horse_number_se], axis=1)
      prediction = model.predict(df).tolist()
      df['競走力指数'] = [x * 100 / sum(prediction) for x in prediction]
      top_odds = race_df['odds'].tolist()
      top_se = pd.Series(top_odds, index=horse_df.index)
      df['単勝オッズ'] = top_se
    else:
      df = None
    return_table = db.session.query(ReturnTableModel).filter(ReturnTableModel.race_id == race_id).first()
